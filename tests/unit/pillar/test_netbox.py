...
def test_when_api_url_is_not_http_or_https_then_error_message_should_be_logged(
    ...
    with patch("saltext.netbox.pillar.netbox.log.error", autospec=True) as fake_error:
...
def test_when_we_retrieve_multiple_devices_then_error_message_should_be_logged(
    ...
    with (
        patch("saltext.netbox.pillar.netbox._get_devices", autospec=True) as multiple_devices,
        patch("saltext.netbox.pillar.netbox.log.error", autospec=True) as fake_error,
    ):
...
def test_when_we_retrieve_multiple_virtual_machines_then_error_message_should_be_logged(
    ...
    with (
        patch(
            "saltext.netbox.pillar.netbox._get_virtual_machines", autospec=True
        ) as multiple_virtual_machines,
        patch("saltext.netbox.pillar.netbox.log.error", autospec=True) as fake_error,
    ):
...
def test_when_we_retrieve_a_device_and_a_virtual_machine_then_error_message_should_be_logged(
    ...
    with (
        patch("saltext.netbox.pillar.netbox._get_devices", autospec=True) as device,
        patch(
            "saltext.netbox.pillar.netbox._get_virtual_machines", autospec=True
        ) as virtual_machine,
        patch("saltext.netbox.pillar.netbox.log.error", autospec=True) as fake_error,
...
def test_when_we_retrieve_no_devices_then_error_message_should_be_logged(
    ...
    with (
        patch("saltext.netbox.pillar.netbox._get_devices", autospec=True) as devices,
        patch("saltext.netbox.pillar.netbox.log.error", autospec=True) as fake_error,
    ):
...
def test_when_we_retrieve_everything_successfully_then_return_dict(
    ...
    with (
        patch("saltext.netbox.pillar.netbox._get_devices", autospec=True) as get_devices,
        patch(
            "saltext.netbox.pillar.netbox._get_virtual_machines", autospec=True
        ) as get_virtual_machines,
        patch("saltext.netbox.pillar.netbox._get_interfaces", autospec=True) as get_interfaces,
        patch(
            "saltext.netbox.pillar.netbox._get_interface_ips", autospec=True
        ) as get_interface_ips,
        patch("saltext.netbox.pillar.netbox._get_site_details", autospec=True) as get_site_details,
        patch(
            "saltext.netbox.pillar.netbox._get_site_prefixes", autospec=True
        ) as get_site_prefixes,
        patch(
            "saltext.netbox.pillar.netbox._get_proxy_details", autospec=True
        ) as get_proxy_details,
        patch(
            "saltext.netbox.pillar.netbox._get_connected_devices", autospec=True
